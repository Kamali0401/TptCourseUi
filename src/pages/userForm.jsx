...
const UserForm = () => {
  ...
  useEffect(() => {
    if (applicationform?.courseID) {
      fetchBatchDropdownReq(applicationform.courseID,"Live").then((res) => {
  ...
  const handleCourseChange = async (courseId, setFieldValue) => {
    ...
    try {
      const response = await fetchBatchDropdownReq(parseInt(courseId),"Live"); // 🔹 your batch API
  ...
  const handleUpdate = async (values, formikHelpers) => {
    ...
    try {
      ...
      // Call your addNewForm API
      await updateForm({
        ...payload, courseId: Number(payload.courseId),  // convert to integer
        batchId: Number(payload.batchId), modifiedBy: "AdminUser"
      }, dispatch);
